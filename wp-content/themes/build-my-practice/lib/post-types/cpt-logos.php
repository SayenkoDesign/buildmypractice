...
class CPT_Logos extends CPT_Core {
    ...
    public function __construct() {
        ...
        $this->capabilities = $this->compile_post_type_capabilities('logo', 'logos' );
	...
	public function columns_display( $column, $post_id ) {     
        ...
        switch ( $column ) {
            ...
            case 'logo_client':
            $logo_client = get_field( 'logo_client', $post_id );
            if( absint( $logo_client ) ) {
                $page = admin_url( 'edit.php' );
                $query_args = array( 'post_type' => self::POST_TYPE, 'client' => $logo_client );
                $page = add_query_arg( $query_args, $page );
                printf( '<a href="%s">%s</a>', $page, get_the_title( $logo_client ) );
            }
            break;
        }
    ...
    function add_extra_tablenav($post_type){
        ...
        $query = $wpdb->prepare('
            ...
            $wpdb->postmeta,
            $wpdb->posts,
            'logo_client', // Your meta key - change as required
    ...
    public function  prefix_parse_filter($query) {
       ...
       if ( is_admin() && 
        ...
        $query->query_vars['meta_key'] = 'logo_client';
