 ...
 *
 * - bundled_item_product_price_html
 * - bundled_item_product_label_title
 *
*/

?><label class="bundled_product_optional_checkbox">
	<input class="bundled_product_checkbox <?php printf( 'bundled_product_checkbox_product-%s', $bundled_item->product->get_id() );?>" type="checkbox" name="<?php echo $bundle_fields_prefix; ?>bundle_selected_optional_<?php echo $bundled_item->get_id(); ?>" value="" <?php checked( $bundled_item->is_optional_checked() && $bundled_item->is_in_stock(), true ); echo $bundled_item->is_in_stock() ? '' : 'disabled="disabled"' ; ?> /> <?php

	$price_html         = apply_filters( 'bundled_item_product_price_html', $bundled_item->product->get_price_html(), $bundled_item );
    ...
    $label_title = apply_filters( 'bundled_item_product_label_title', __( 'add' ) . $label_title, $bundled_item );

	echo sprintf( __( '%1$s%2$s%3$s', 'woocommerce-product-bundles' ), $label_title, $label_price, $label_stock_status );
