...
<div id="primary" class="content-area">
	...
	function service_tab_links() {
        ...
        if( false != ( $link = $my_account->get_business_card_link() ) ) {
            $args = array( 'title' => 'Business Cards', 'href' => sprintf( '%s#%s', get_permalink( $link ), $fa_tabs->settings['id'] ) );
            $fa_tabs->add_tab_link( $args );
        }
        
        if( false != ( $link = $my_account->get_logo_link() ) ) {
            $args = array( 'title' => 'Logos', 'href' => sprintf( '%s#%s', get_permalink( $link ), $fa_tabs->settings['id'] ), 'active' => true );
            $fa_tabs->add_tab_link( $args );
        }
